...
func GetUavByUid(Uid string) Model.Uav {
	var uav Model.Uav
...
func GetBackUavByUid(Uid string) (bool, Model.BackUav) {
	var uav Model.BackUav

	DB := db.Model(&Model.Uav{}).Where(&Model.Uav{Uid: Uid}).First(&uav)
	if DB.Error != nil {
		fmt.Println("获取对应序列号的设备返回信息：", DB.Error.Error())
		return false, Model.BackUav{}
	}
...
func GetUavsByUids(Uids []string) []Model.BackUav {
	var uavs []Model.BackUav
...
func InsertUva(UavName string, UavType string, UavUid string) (bool, string) {
	...
	//创建新记录
	DB = db.Create(&Model.Uav{Name: UavName, Type: UavType, Uid: UavUid, Get_time: time.Unix(0, 0), Plan_time: time.Unix(0, 0), Back_time: time.Unix(0, 0)})
...
func GetUavByAll(uav Model.SearchUav) []Model.Uav {
...
func GetUavStateByUid(u Model.BorrowUav) string {
	var uav Model.Uav
...
func SearchStuInOneDay() ([]Model.Uav, bool) {
	var uavs []Model.Uav
	DB := db.Model(&Model.Uav{}).Where(&Model.Uav{State: "using"}).Where("plan_time > ?", time.Now().AddDate(0, 0, -2)).Find(&uavs)
	if DB.Error != nil {
...
func GetUavsByStatesWithPage(UavState string, UavType string, Page string, Max int) []Model.BackUav {
...
func UpdateBorrower(UavUid string, UavBorrower string, UavPhone string, UavStuID string) {
	DB := db.Model(&Model.Uav{}).Where(&Model.Uav{Uid: UavUid}).Updates(Model.Uav{Borrower: UavBorrower, Phone: UavPhone, StudentID: UavStuID})
...
func UpdateBorrowTime(UavUid string, BorrowTime time.Time) {
	...
	DB := db.Model(&Model.Uav{}).Where(&Model.Uav{Uid: UavUid}).Updates(Model.Uav{Get_time: BorrowTime})
...
func UpdatePlanTime(UavUid string, UavPlanTime time.Time) {
	DB := db.Model(&Model.Uav{}).Where(&Model.Uav{Uid: UavUid}).Updates(Model.Uav{Plan_time: UavPlanTime})
...
func UpdateBackTime(UavUid string) {
	...
	db.Model(&Model.Uav{}).Where(&Model.Uav{Uid: UavUid}).First(&uav).Updates(Model.Uav{Back_time: time.Now()})
