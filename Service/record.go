...
func RecordBorrow(Uid string, Stuid string, Borrower string, Plan_time time.Time, Usage string) bool {
	...
	DB := db.Create(&Model.Record{Uid: Uid, StudentID: Stuid, Borrower: Borrower, Plan_time: Plan_time, Usage: Usage, Get_time: time.Now(), Back_time: time.Unix(0, 0), GetReviewTime: time.Unix(0, 0), BackReviewTime: time.Unix(0, 0)}).Select("id").Find(&id)
...
func UpdateGetTimeinRecords(Uid string) bool {
	...
	DB := db.Model(&Model.Record{}).Where("id", id).Updates(&Model.Record{Get_time: time.Now()})
	if DB.Error != nil {
...
func GetRecordsByID(Stuid string) []Model.BackRecord {
	...
	for i, t := range times {
		...
		DB = db.Model(&Model.Record{}).Where(&Model.Record{Get_time: t, StudentID: Stuid}).Select("state, uid, get_time, back_time, plan_time").Find(&uavs)
		if DB.Error != nil {
		...
		DB = db.Model(&Model.Record{}).Where(&Model.Record{Get_time: t}).First(&uavpack)
		if DB.Error != nil {
		...
		//判断本次借用状态
		var states []string
		DB = db.Model(&Model.Record{}).Where(&Model.Record{Get_time: t, StudentID: Stuid}).Select("state").Find(&states)
		if DB.Error != nil {
...
func GetReviewRecord(Uid string, Checker string, Result string, Comment string, GetTime time.Time) bool {
	...
	DB := db.Model(&Model.Record{}).Where("id", id).Updates(&Model.Record{GetReviewer: Checker, Get_time: GetTime, GetReviewResult: Result, GetReviewComment: Comment, GetReviewTime: time.Now()})
	if DB.Error != nil {
		fmt.Println("添加借用审核记录失败：", DB.Error.Error())
		return false
	}
...
func UpdateBackRecord(Uid string) bool {
	...
	DB := db.Model(&Model.Record{}).Where("id", id).Updates(&Model.Record{Back_time: time.Now()})
	if DB.Error != nil {
		fmt.Println("添加归还时间失败：", DB.Error.Error())
		return false
	}
